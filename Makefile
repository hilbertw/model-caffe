...
CFLAGS:=$(CFLAGS) -I/usr/local/include -I/usr/local/include/opencv

SYSTEMC_INSTALL:= /disk1/systemc/install

CFLAGS:=$(CFLAGS) --std=c++11  -I $(CAFFE_SOURCE)/include -I$(SC_SOURCE)/gen -I$(SC_SOURCE)/src -I$(SYSTEMC_INSTALL)/include
...
ALL_OBJS:=$(addprefix obj/,$(OBJS) $(EXT_OBJS) $(SC_OBJS) $(WEIGHT_OBJS))
